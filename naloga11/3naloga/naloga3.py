...
# Sum meritve
def sigma_v(v):
    return max(1/3.6, LA.norm(v)**2*0.01**2)
...
def Qn(x, v, P, a):
    ...
    Qn = dt**2 * (sigma_a**2 * I + np.dot(np.dot(vT, pn), vT)/norma**4 * 
            np.outer(np.dot(Bn(v), aT), np.dot(Bn(v), aT)))
    return Qn

# Zacetni vektor stanja 
#x = np.ones(4)
x = np.array([x0,y0,1,1])
trajektorija = x

# Zacetna kovarianca matrika
sigmav0 = sigma_v(x[2:4])
P = np.diag([sigma_x**2, sigma_x**2, sigmav0**2, sigmav0**2])
#P =  np.ones(shape=(4, 4))
#enke = np.ones(shape=(2,2))
#P =  np.block([[enke, nicle],
#               [nicle, enke*10**4]])
...
for i in range(1,n):
    ...
    Q = np.block([[nicle, nicle], 
                  [nicle, Qn(s, v, P, pospesek[i])]])
...
"""
### Printam trajektorijo in hitrosti
n = len(t)
for i in range(n):
    x,y,vx,vy = trajektorija[i]
    print(t[i], x,y,vx,vy, sep ='\t')
"""
### Printam odstopanja od pravih vrednosti
n = len(t)
for i in range(1,n):
    dr1 = Z[i] - pozicija[i]
